...
const Sidebar = () => {
    ...
    return (
        <div className={` h-screen flex`}>
            <aside className={` flex  flex-col   z-50  `} style={{ width: width ? "350px" : 70 }} >
                                                        ...
                                                        <div className='flex '>
                                                            <Tooltip placement={'right'} className='flex' title={subItem.menuTitle}>
                                                                <div onClick={(e) => openSubToSubMenu(subItem.id)} className={` flex w-full  `}>
                                                                    <NavLink to={subItem?.link} onClick={() => setWidth(true)} className={` ${pathname === subItem.menuTitle ? 'bg-black text-[#f07e01]' : ""} flex items-center justify-start textHover gap-4 text-white cursor-pointer hover:text-[#f07e01] my-1  rounded-r-lg px-5 py-2`}
                                                                        style={width ? { width: '70%' } : { width: '100%' }}
                                                                    >
                                                                        <div className='p-2 border rounded-md bg-white text-gray-500 text-sm'>
                                                                            {subItem.icon}
                                                                        </div>
                                                            ...
                                                            <>
                                                                {
                                                                    ...
                                                                    subItem.subintoSubData.map((subintoSubData, subintoSubIndex) => {
                                                                        return (
                                                                            <div key={subintoSubData.id} style={clicked === subItem.id ? { display: 'flex' } : { display: 'none' }} className='flex transition-all text-black  items-center justify-start pl-14'>
                                                                                <ul className='list-disc'>

                                                                                    <li className={'transition-all text-gray-400  hover:text-[#f07e01] mt-1 '}>
                                                                                        <NavLink to={subintoSubData.link} className={''} >
                                                                                            {subintoSubData.name}
                                                                                        </NavLink>
                                                                                    </li>
                                                                                </ul>
